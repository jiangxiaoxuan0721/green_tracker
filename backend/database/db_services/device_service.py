...
def create_device(db: Session, device_type: str, platform_level: str, 
                 ...
                 description: str = "", owner_id: str = "") -> Device:
    """
    创建新设备
    ...
    Returns:
        Device: 创建的设备对象
    """
    print(f"[后端DeviceService] 开始创建设备: 类型={device_type}, 平台={platform_level}, 所有者ID={owner_id}")
...
def get_devices_by_owner(db: Session, owner_id: str, active_only: bool = True) -> List[Device]:
    """
    根据所有者ID获取设备列表
...
def search_devices(db: Session, owner_id: str = "", device_type: str = "",
...
def update_device(db: Session, device_id: str, owner_id: str = "",
    ...
    if not device:
        print("[后端DeviceService] 设备不存在或无权限访问")
        return None
...
def delete_device(db: Session, device_id: str, owner_id: str = "", soft_delete: bool = True) -> bool:
    """
    删除设备
...
def restore_device(db: Session, device_id: str, owner_id: str = "") -> Optional[Device]:
    """
    恢复已软删除的设备
    ...
    if not device:
        print("[后端DeviceService] 已删除的设备不存在或无权限访问")
        return None
