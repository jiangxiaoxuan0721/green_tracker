...
def send_verification_email(to_email: str, code: str) -> bool:
    ...
    if not SMTP_HOST or not SMTP_USER or not SMTP_PASSWORD:
        logger.warning("SMTP 未配置，验证码将不会实际发送。验证码: %s -> %s", code, to_email)
        return True  # 开发环境下允许通过

    try:
        msg = MIMEMultipart()
        msg["Subject"] = f"您的验证码：{code} - {SMTP_FROM_NAME}"
        ...
        <div style="max-width:480px;margin:0 auto;font-family:Arial,sans-serif;">
            ...
            <div style="padding:24px;background:#fff;border:1px solid #e5e7eb;border-top:none;border-radius:0 0 8px 8px;">
                <p style="margin:0 0 16px;color:#374151;font-size:14px;">您正在注册 {SMTP_FROM_NAME} 账号，以下是您的验证码：</p>
                <div style="text-align:center;padding:16px;background:#f0fdf4;border-radius:6px;margin-bottom:16px;">
                    <span style="font-size:28px;font-weight:700;letter-spacing:6px;color:#16a34a;">{code}</span>
                </div>
