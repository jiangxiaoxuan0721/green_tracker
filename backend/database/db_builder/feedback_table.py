...
提供创建和删除反馈表的功能
使用方法：
1. 从backend目录运行：python -m database.db_builder.feedback_table create
2. 从backend目录运行：python -m database.db_builder.feedback_table drop
"""
    ...
    try:
        ...
        try:
            ...
            with engine.connect() as conn:
                ...
                # 在用户schema中创建表
                Feedback.__table__.create(engine, checkfirst=True)
                print(f"在用户{current_user}的schema中创建反馈表成功！")
                return True
...
def drop_feedback_table():
    ...
    """
    confirm = input("确认删除反馈表？这将删除所有数据(y/n): ")
    if confirm.lower() == 'y':
        print("正在删除反馈表...")
...
def reset_feedback_table():
    ...
    注意：此操作将删除表中的所有数据，请谨慎使用！
    """
    confirm = input("确认重置反馈表？这将删除所有数据并重新创建表(y/n): ")
    if confirm.lower() == 'y':
        print("正在重置反馈表...")
        Feedback.__table__.drop(engine, checkfirst=True)
        Feedback.__table__.create(engine, checkfirst=True)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        if command == "create":
            ...
            print("用法: python -m database.db_builder.feedback_table [create|drop|reset]")
    else:
        print("用法: python -m database.db_builder.feedback_table [create|drop|reset]")